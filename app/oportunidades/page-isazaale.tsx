...
import { Plus, Search, Filter, Briefcase, Trash2, ArrowUpDown, ChevronUp, ChevronDown, ChevronRight, Pencil } from "lucide-react";
            ...
            <div className="flex flex-col relative min-h-[450px] transition-all duration-300">
                {loading && opportunities.length === 0 ? (
                    ...
                    <>
                        ...
                        {/* VISTA DESKTOP: Tabla Moderna Premium */}
                                ...
                                <table className="w-full text-left border-collapse">
                                    <thead>
                                        <tr className="border-b border-slate-200 text-[11px] font-bold text-slate-400 uppercase bg-slate-50/50">
                                            {userRole === 'ADMIN' && (
                                                <th className="px-4 py-3 text-center w-[100px] select-none">
                                                    Acciones
                                                </th>
                                            )}
                                            <th className="px-4 py-3 font-bold select-none cursor-pointer group" onClick={() => handleSort('account_name')}>
                                                <div className="flex items-center">Cuenta <SortIcon field="account_name" /></div>
                                            </th>
                                            <th className="px-4 py-3 font-bold select-none cursor-pointer group" onClick={() => handleSort('nombre')}>
                                                <div className="flex items-center">Oportunidad <SortIcon field="nombre" /></div>
                                            </th>
                                            <th className="px-4 py-3 font-bold select-none cursor-pointer group" onClick={() => handleSort('fase_name')}>
                                                <div className="flex items-center">Fase <SortIcon field="fase_name" /></div>
                                            </th>
                                            <th className="px-4 py-3 font-bold select-none cursor-pointer group" onClick={() => handleSort('estado_name')}>
                                                <div className="flex items-center">Estado <SortIcon field="estado_name" /></div>
                                            </th>
                                            <th className="px-4 py-3 font-bold text-center select-none cursor-pointer group" onClick={() => handleSort('created_at')}>
                                                <div className="flex items-center justify-center">Creada <SortIcon field="created_at" /></div>
                                            </th>
                                            <th className="px-4 py-3 font-bold text-right select-none cursor-pointer group" onClick={() => handleSort('amount')}>
                                                <div className="flex items-center justify-end">Valor <SortIcon field="amount" /></div>
                                            </th>
                                            <th className="px-4 py-3 font-bold text-center select-none cursor-pointer group" onClick={() => handleSort('fecha_cierre_estimada')}>
                                                <div className="flex items-center justify-center">Cierre <SortIcon field="fecha_cierre_estimada" /></div>
                                            </th>
                                            <th className="px-4 py-3 font-bold select-none cursor-pointer group" onClick={() => handleSort('vendedor_nombre')}>
                                            ...
                                            return (
                                                <tr 
                                                    ...
                                                    className={cn(
                                                        ...
                                                        isOverdue ? "bg-red-50/40 hover:bg-red-50/60" : "bg-white"
                                                    )}
                                                ...
                                                >
                                                    ...
                                                    <td className="px-4 py-3">
                                                        <span className="text-xs font-semibold text-blue-600 truncate block max-w-[150px]" title={opp.account?.nombre || ""}>
                                                            {opp.account?.nombre || "Sin cuenta"}
                                                        </span>
                                                    </td>
                                                    <td className="px-4 py-3">
                                                        <span className="text-xs font-bold text-slate-800 truncate block max-w-[200px]" title={opp.nombre || ""}>
                                                            {opp.nombre || "Sin nombre"}
                                                        </span>
                                                    </td>
                                                    <td className="px-4 py-3">
                                                    ...
                                                    <td className="px-4 py-3">
                                                        <span className={cn(
                                                            "text-[10px] px-2 py-0.5 rounded-full font-bold uppercase whitespace-nowrap border",
                                                            (opp.estado_data?.nombre?.toLowerCase().includes('ganada') || opp.estado_data?.nombre?.toLowerCase().includes('ganado')) ? "bg-emerald-50 text-emerald-700 border-emerald-200" :
                                                            (opp.estado_data?.nombre?.toLowerCase().includes('perdida') || opp.estado_data?.nombre?.toLowerCase().includes('perdido') || opp.estado_data?.nombre?.toLowerCase().includes('cancelada')) ? "bg-rose-50 text-rose-700 border-rose-200" :
                                                    ...
                                                    <td className="px-4 py-3 text-center">
                                                        <div className={cn(
                                                            ...
                                                            <span className="text-xs font-semibold leading-none">
                                                                {opp.fecha_cierre_estimada ? new Date(opp.fecha_cierre_estimada).toLocaleDateString('es-ES', { day: '2-digit', month: '2-digit', year: 'numeric' }) : "-"}
                                                            </span>
                                                            {isOverdue && <span className="text-[9px] font-black uppercase leading-none mt-0.5 tracking-wider">Vencido</span>}
                                                        </div>
                                                    </td>
                                                    <td className="px-4 py-3">
                                                        <div className="flex items-center gap-1.5 max-w-[150px]">
                                                            <div className="w-5 h-5 shrink-0 rounded-full bg-indigo-50 text-indigo-700 flex items-center justify-center text-[9px] font-black border border-indigo-150">
                                                                {getInitials(opp.vendedor?.full_name || "Sin")}
                                                            </div>
                                                            <span className="text-xs text-slate-600 truncate" title={opp.vendedor?.full_name || ""}>

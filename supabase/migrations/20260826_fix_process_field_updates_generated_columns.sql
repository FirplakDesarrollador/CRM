-- Migration: 20260826_fix_process_field_updates_generated_columns.sql
-- Prevent attempting to insert or update generated columns (is_generated = 'ALWAYS') like CRM_CotizacionItems.subtotal
        ...
        BEGIN -- START INDIVIDUAL UPDATE BLOCK
            IF v_field = '_complete_snapshot_' AND v_current_row IS NULL THEN
                ...
                LOOP
                    ...
                    v_col_type := NULL;
                    v_is_generated := NULL;
                    SELECT udt_name, is_generated INTO v_col_type, v_is_generated 
                    FROM information_schema.columns 
                    WHERE table_schema = 'public' AND table_name = p_table_name AND column_name = v_snap_field;

                    IF v_col_type IS NOT NULL AND COALESCE(v_is_generated, 'NEVER') <> 'ALWAYS' THEN
                        v_insert_cols := array_append(v_insert_cols, quote_ident(v_snap_field));
                        -- Robust casting for snapshot fields (handling array types like _uuid, _text, etc.)
                        IF LEFT(v_col_type, 1) = '_' THEN
                            v_base_col_type := substring(v_col_type from 2);
                            v_insert_placeholders := array_append(v_insert_placeholders, format('(CASE WHEN ($2->%L) IS NULL OR jsonb_typeof($2->%L) = ''null'' THEN NULL WHEN jsonb_typeof($2->%L) = ''array'' THEN (SELECT COALESCE(array_agg(elem::%I), ARRAY[]::%I[]) FROM jsonb_array_elements_text($2->%L) elem) ELSE ($2->>%L)::%I END)', v_snap_field, v_snap_field, v_snap_field, v_base_col_type, v_base_col_type, v_snap_field, v_snap_field, v_col_type));
                        ELSE
                ...
                LOOP
                    ...
                    IF v_ts > v_last_ts THEN
                        ...
                        IF v_col_type IS NOT NULL AND COALESCE(v_is_generated, 'NEVER') <> 'ALWAYS' THEN
                            IF LEFT(v_col_type, 1) = '_' THEN
                                v_base_col_type := substring(v_col_type from 2);
                                v_update_clauses := array_append(v_update_clauses, format('%I = (CASE WHEN ($1->%L) IS NULL OR jsonb_typeof($1->%L) = ''null'' THEN NULL WHEN jsonb_typeof($1->%L) = ''array'' THEN (SELECT COALESCE(array_agg(elem::%I), ARRAY[]::%I[]) FROM jsonb_array_elements_text($1->%L) elem) ELSE ($1->>%L)::%I END)', v_snap_field, v_snap_field, v_snap_field, v_snap_field, v_base_col_type, v_base_col_type, v_snap_field, v_snap_field, v_col_type));
                            ELSE
                ...
                IF v_current_row IS NULL THEN
                    ...
                    IF v_field NOT IN (v_id_col, 'created_by', 'updated_by', '_sync_metadata', 'created_at', 'updated_at') THEN
                        ...
                        IF v_col_type IS NOT NULL AND COALESCE(v_is_generated, 'NEVER') <> 'ALWAYS' THEN
                            ...
                            IF LEFT(v_col_type, 1) = '_' THEN
                                v_base_col_type := substring(v_col_type from 2);
                                v_insert_placeholders := array_append(v_insert_placeholders, format('(CASE WHEN ($2) IS NULL OR jsonb_typeof($2) = ''null'' THEN NULL WHEN jsonb_typeof($2) = ''array'' THEN (SELECT COALESCE(array_agg(elem::%I), ARRAY[]::%I[]) FROM jsonb_array_elements_text($2) elem) ELSE ($2#>>''{}'')::%I END)', v_base_col_type, v_base_col_type, v_col_type));
                            ELSE
                    ...
                    IF v_ts > v_last_ts THEN
                        ...
                        IF v_col_type IS NOT NULL AND COALESCE(v_is_generated, 'NEVER') <> 'ALWAYS' THEN
                            ...
                            IF v_field NOT IN ('id', 'uuid_generado', '_sync_metadata', 'created_at', 'updated_at', 'created_by', 'updated_by') THEN
                                IF LEFT(v_col_type, 1) = '_' THEN
                                    v_base_col_type := substring(v_col_type from 2);
                                    v_update_clauses := array_append(v_update_clauses, format('%I = (CASE WHEN ($1) IS NULL OR jsonb_typeof($1) = ''null'' THEN NULL WHEN jsonb_typeof($1) = ''array'' THEN (SELECT COALESCE(array_agg(elem::%I), ARRAY[]::%I[]) FROM jsonb_array_elements_text($1) elem) ELSE ($1#>>''{}'')::%I END)', v_field, v_base_col_type, v_base_col_type, v_col_type));
                                ELSE
                            ...
                            v_success := TRUE;
                            v_message := 'Updated';
                        ELSIF v_col_type IS NOT NULL AND COALESCE(v_is_generated, 'NEVER') = 'ALWAYS' THEN
                            v_success := TRUE;
                            v_message := 'Skipped (Generated column)';
                        ELSE
                            v_success := FALSE;

-- Migration: Add origin filter to get_sales_funnel_data RPC
-- Description: Allows filtering the sales funnel in the dashboard by opportunity origin (origen_oportunidad)

DROP FUNCTION IF EXISTS public.get_sales_funnel_data(uuid, text, text, uuid, integer, text, text, date, date);

CREATE OR REPLACE FUNCTION public.get_sales_funnel_data(
    p_user_id uuid,
    p_user_role text,
    p_canal_id text DEFAULT NULL::text,
    p_advisor_id uuid DEFAULT NULL::uuid,
    p_subclasificacion_id integer DEFAULT NULL::integer,
    p_nivel_premium text DEFAULT NULL::text,
    p_search_query text DEFAULT NULL::text,
    p_date_from date DEFAULT NULL::date,
    p_date_to date DEFAULT NULL::date,
    p_origen_oportunidad text DEFAULT NULL::text
)
RETURNS TABLE (
    fase_id integer,
    fase_nombre text,
...
BEGIN
    ...
    IF p_canal_id IS NOT NULL THEN
        ...
        FilteredOpps AS (
            ...
            JOIN "CRM_FasesOportunidad" f_opp ON o.fase_id = f_opp.id
            WHERE 
                o.is_deleted = false
                AND (
                    (o.estado_id = 1 OR o.estado_id IS NULL)
                    OR
                    (o.estado_id IN (2, 3, 11, 14) AND f_opp.nombre ILIKE 'Cerrada %')
                )
                AND (
                    p_user_role = 'ADMIN'
                ...
                AND (
                    p_origen_oportunidad IS NULL
                    OR (
                        CASE 
                            WHEN lower(p_origen_oportunidad) = 'wp' THEN (o.origen_oportunidad ILIKE '%wp%' OR o.origen_oportunidad ILIKE '%whatsapp%')
                            ELSE o.origen_oportunidad ILIKE '%' || p_origen_oportunidad || '%'
                        END
                    )
                )
        )

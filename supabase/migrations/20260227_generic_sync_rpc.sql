-- =============================================================================
-- MIGRATION: 20260227_generic_sync_rpc.sql
-- FIX: process_field_updates - Generic Validation to Prevent Synchronization Loops
-- =============================================================================
...
BEGIN
    -- 0. Get all valid columns for the table to avoid failing on invalid frontend fields
    SELECT array_agg(column_name::text) INTO v_valid_columns
    FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = p_table_name;

    v_has_owner_col := 'owner_user_id' = ANY(v_valid_columns);
    v_has_user_col := 'user_id' = ANY(v_valid_columns);
    ...
    LOOP
        ...
        IF v_current_row IS NULL THEN
            ...
            BEGIN
                ...
                LOOP
                    -- Verify the column actually exists in the database
                    IF v_update.field = ANY(v_valid_columns) THEN
                    ...
                    ELSE
                        -- Quietly skip non-existent columns (forces `success: true` to prevent infinite sync loops)
                        v_results := v_results || jsonb_build_object('id', v_id, 'field', v_update.field, 'success', true, 'message', 'Skipped (Column does not exist)');
                    END IF;
                END LOOP;
                ...
                v_results := v_results || jsonb_build_object('id', v_id, 'field', '_all', 'success', true, 'message', 'Created');
            EXCEPTION WHEN OTHERS THEN
                v_results := v_results || jsonb_build_object('id', v_id, 'field', '_all', 'success', false, 'message', SQLERRM);
            END;
        ELSE
            -- ==========================================
            ...
            LOOP
                BEGIN
                    -- Verify the column actually exists in the database
                    IF NOT (v_update.field = ANY(v_valid_columns)) THEN
                        v_results := v_results || jsonb_build_object('id', v_id, 'field', v_update.field, 'success', true, 'message', 'Skipped (Column does not exist)');
                        CONTINUE;
                    END IF;
                    ...
                    IF v_update.ts > v_last_ts THEN
                        ...
                        v_results := v_results || jsonb_build_object('id', v_id, 'field', v_update.field, 'success', true, 'message', 'Updated');
                    ELSE
                        v_results := v_results || jsonb_build_object('id', v_id, 'field', v_update.field, 'success', true, 'message', 'Skipped (Outdated)');
                    END IF;
                EXCEPTION WHEN OTHERS THEN
                    v_results := v_results || jsonb_build_object('id', v_id, 'field', v_update.field, 'success', false, 'message', SQLERRM);
                END;
            END LOOP;
        END IF;
    END LOOP;

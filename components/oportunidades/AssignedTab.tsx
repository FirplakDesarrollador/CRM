...
export function AssignedTab({ opportunityId, currentOwnerId }: AssignedTabProps) {
    ...
    const handleAssign = async () => {
        ...
        // if (selectedUserId === currentOwnerId) return; // Allow re-assigning to same (maybe to trigger sync?) No, user wants re-assignment.

        setIsSaving(true);
        setSuccessMessage(null);

        try {
            await updateOpportunity(opportunityId, { owner_user_id: selectedUserId });
            setSuccessMessage("Oportunidad reasignada correctamente.");
        ...
        <div className="space-y-6">
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                <div className="flex items-center gap-3 mb-6">
                    ...
                    <div>
                        <h3 className="font-bold text-slate-900 text-lg">Reasignar Oportunidad</h3>
                        <p className="text-xs text-slate-500">Cambiar el propietario de esta oportunidad</p>
                    </div>
                </div>

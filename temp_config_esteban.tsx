...
export default function ConfigPage() {
    ...
    const clearOutbox = async () => {
        if (confirm('┬┐Est├ís seguro de limpiar la cola de sincronizaci├│n? Los cambios locales no sincronizados se perder├ín.')) {
    ...
    const resetLocalData = async () => {
        if (confirm('PELIGRO: Esto borrar├í TODOS los datos locales y reiniciar├í la aplicaci├│n. Usar solo si hay errores persistentes que no se resuelven sincronizando.\n\n┬┐Desea continuar?')) {
            try {
                await db.delete();
                alert('Base de datos eliminada. La aplicaci├│n se recargar├í para resincronizar datos limpios.');
        ...
        <div className="p-6 max-w-5xl mx-auto space-y-6">
            ...
            <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                    ...
                    <p className="text-slate-500 font-medium">Estado del sistema y herrmientas de diagn├│stico</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="md:col-span-2 bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
                    <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                        <div className="flex items-center gap-2">
                            ...
                            <h3 className="font-bold text-slate-900 text-lg">Sincronizaci├│n</h3>
                        </div>
                        <button
                            onClick={forceSync}
                            disabled={isSyncing}
                    ...
                    <div className="p-6 space-y-6">
                        <div className="grid grid-cols-2 gap-4">
                            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                                <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">├Ültima Sincronizaci├│n</p>
                                <p className="text-lg font-bold text-slate-900">
                                    {lastSyncTime ? new Date(lastSyncTime).toLocaleTimeString() : 'Nunca'}
                                </p>
                            </div>
                            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                                <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Pendientes de Env├¡o</p>
                                <p className={cn(
                                    "text-lg font-bold",
                        ...
                        {!error && !isSyncing && pendingCount === 0 && (
                            <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-4 flex items-center gap-3">
                                ...
                                <p className="text-sm font-bold text-emerald-900">Todo el contenido est├í sincronizado con la nube.</p>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-4 mt-4">
                        <button
                            onClick={async () => {
                                if (confirm("Esto forzar├í la recarga de la configuraci├│n (Fases, Canales) desde el servidor. ┬┐Continuar?")) {
                                    await db.phases.clear();
                                    await syncEngine.triggerSync();
                                    alert("Sincronizaci├│n forzada iniciada. Por favor espera a que termine.");
                                }
                            }}
...
function AdminSettings() {
    ...
    const handleSave = async () => {
        ...
        if (success) alert('Configuraci├│n guardada correctamente');
        setIsSaving(false);
    };
    ...
    return (
        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden p-6 space-y-6">
            <div className="flex items-center gap-2">
                ...
                <div>
                    <h3 className="font-bold text-slate-900 text-lg">Configuraci├│n de Administrador</h3>
                    <p className="text-sm text-slate-500">Par├ímetros globales del sistema</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                    <label className="block text-sm font-bold text-slate-700 mb-2">
                        Pedido M├¡nimo Cliente Premium (COP)
                    </label>
                    <p className="text-xs text-slate-500 mb-3">
                        Valor m├¡nimo requerido en cotizaciones para clientes marcados como Premium. Si no se cumple, no podr├ín generar pedido.
                    </p>
